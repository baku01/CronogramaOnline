...
import { calculateEndDate, getWorkingDaysBetween } from "../utils/dateCalculations";
...
const TaskModal: React.FC<TaskModalProps> = ({ isOpen, onClose, onSave, task, resources, allTasks = [], dependencies = [], calendars = [] }) => {
    ...
    useEffect(() => {
        if (isOpen) {
            if (task) {
                // Determine calendar for initial calculation
                const effectiveCalendar = task.calendarId
                    ? availableCalendars.find(c => c.id === task.calendarId)
                    : (availableCalendars.find(c => c.isDefault) || availableCalendars[0]);

                const duration = task.duration || getWorkingDaysBetween(task.start, task.end, effectiveCalendar);
                setFormData({
                    ...task,
                ...
                const defaultCalendar = availableCalendars.find(c => c.isDefault) || availableCalendars[0];
                const now = new Date();
                const tomorrow = calculateEndDate(now, 1, defaultCalendar); // 1 day duration default
    ...
    const getEffectiveCalendar = (currentFormData: Partial<ExtendedTask>) => {
        if (currentFormData.calendarId) {
            return availableCalendars.find(c => c.id === currentFormData.calendarId);
        }
        return availableCalendars.find(c => c.isDefault) || availableCalendars[0];
    };

    const handleChange = (field: keyof ExtendedTask, value: any) => {
        setFormData((prev) => {
            const newData = { ...prev, [field]: value };
            const effectiveCalendar = getEffectiveCalendar(newData);

            // Auto-calculate dates if duration changed
            if (field === 'duration' && typeof value === 'number') {
                if (prev.start) {
                    newData.end = calculateEndDate(prev.start, value, effectiveCalendar);
                }
            }

            // Auto-calculate duration if end date changed
            if (field === 'end' && value instanceof Date && prev.start) {
                newData.duration = getWorkingDaysBetween(prev.start, value, effectiveCalendar);
            }

             // Auto-calculate duration if start date changed
            if (field === 'start' && value instanceof Date && prev.end) {
                 if (prev.duration) {
                     newData.end = calculateEndDate(value, prev.duration, effectiveCalendar);
                 } else {
                     newData.duration = getWorkingDaysBetween(value, prev.end, effectiveCalendar);
                 }
            }

            // Recalculate if calendar changed
            if (field === 'calendarId') {
                 if (prev.start && prev.duration) {
                     newData.end = calculateEndDate(prev.start, prev.duration, effectiveCalendar);
                 }
            }
